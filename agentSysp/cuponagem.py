...
def processacupom(numpedecf):
    ...
    cursor_postgre.execute(f'''
        ...
        FROM 
            cpfcli_campanha
        WHERE 
            ativo = 'S' and usa_numero_da_sorte = 'N'
    ''')
    campanha = cursor_postgre.fetchone()

    if campanha is None:
        return 'Não existe campanha ativa que use cuponagem'
    ...
    if list_numpeds and len(list_numpeds) > 0:
        ...
        not_in_clauses = build_clause("AND PCPEDCECF.NUMPEDECF", numped_list_postgre, 'NOT')
    else:
        not_in_clauses = ''
    ...
    print(qtcupons)
    cursor.execute(f'''
        SELECT CODCLI, CLIENTE, EMAIL, CGCENT FROM PCCLIENT WHERE CODCLI = {ped[3]}
    ''')
